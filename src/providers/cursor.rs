...
mod tests {
    ...
    fn new_uses_env_override() {
        ...
        unsafe { std::env::set_var(CURSOR_PATH_ENV, "/usr/local/bin/cursor") };
        let provider = CursorProvider::new();
        assert_eq!(provider.cursor_path, PathBuf::from("/usr/local/bin/cursor"));
        match orig {
            Some(v) => unsafe { std::env::set_var(CURSOR_PATH_ENV, v) },
            None => unsafe { std::env::remove_var(CURSOR_PATH_ENV) },
        }
    }
    ...
    fn new_defaults_to_cursor() {
        let _guard = env_lock();
        let orig = std::env::var(CURSOR_PATH_ENV).ok();
        unsafe { std::env::remove_var(CURSOR_PATH_ENV) };
        let provider = CursorProvider::new();
        assert_eq!(provider.cursor_path, PathBuf::from("cursor"));
        if let Some(v) = orig {
            unsafe { std::env::set_var(CURSOR_PATH_ENV, v) };
        }
    }
    ...
    fn new_ignores_blank_env_override() {
        let _guard = env_lock();
        let orig = std::env::var(CURSOR_PATH_ENV).ok();
        unsafe { std::env::set_var(CURSOR_PATH_ENV, "   ") };
        let provider = CursorProvider::new();
        assert_eq!(provider.cursor_path, PathBuf::from("cursor"));
        match orig {
            Some(v) => unsafe { std::env::set_var(CURSOR_PATH_ENV, v) },
