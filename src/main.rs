...
Examples:
  ...
  zeroclaw gateway --new-pairing 123456  # clear tokens and set pairing code")]
    Gateway {
        ...
        /// Clear all paired tokens and set a new pairing code (6-digit numeric)
        #[arg(long, value_name = "CODE")]
        new_pairing: Option<String>,
...
List, inspect, store, and clear memory entries stored by the agent. \
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Gateway {
            ...
            if let Some(code) = new_pairing {
                if code.len() != 6 || !code.chars().all(|c| c.is_ascii_digit()) {
                    bail!("--new-pairing CODE must be exactly 6 ASCII digits (e.g. 123456)");
                }
                // Persist token reset + pairing code so env-derived overrides are not written to disk.
                let mut persisted_config = Config::load_or_init().await?;
                persisted_config.gateway.paired_tokens.clear();
                persisted_config.gateway.pairing_code = Some(code);
                persisted_config.save().await?;
                config.gateway.paired_tokens.clear();
                info!(
                    "🔐 Cleared paired tokens — pairing code set (will activate on next gateway start)"
                );
                return Ok(());
            }
                        ...
                        match auth::gemini_oauth::start_device_code_flow(&client).await {
                            ...
                            Err(e) => {
                                    ...
                                    || err_msg.contains("Cloudflare")
                                {
                                    ...
                                    println!(
                                        "   Check your GEMINI_OAUTH_CLIENT_ID and GEMINI_OAUTH_CLIENT_SECRET"
                                    );
                                } else {
...
mod tests {
    ...
    fn gateway_cli_accepts_new_pairing_with_code() {
        ...
        match cli.command {
            Commands::Gateway { new_pairing, .. } => {
                assert_eq!(new_pairing.as_deref(), Some("123456"));
            }
    ...
    // update_help_mentions_instructions_flag test removed:
    // the `update` subcommand no longer exists.
}
