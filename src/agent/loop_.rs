        ...
        if tool_calls.is_empty() {
            ...
            if missing_tool_call_retry_used && !tool_specs.is_empty() && missing_tool_call_signal {
                runtime_trace::record_event(
                    ...
                    Some(false),
                    Some(
                        "llm response still implied follow-up action but emitted no tool call after retry",
                    ),
                    serde_json::json!({
                        "iteration": iteration + 1,
                        "response_excerpt": truncate_with_ellipsis(&redact_trace_text(&display_text), 600),
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    if is_loop_detection_error(&e) {
                        let notice = "\u{26a0}\u{fe0f} Loop pattern detected and agent stopped early. \
...
http_request/url>https://example.com"#;
    ...
    // parse_glm_style_rejects_non_http_url_param was removed: the
    // GLM-style URL-value sanitisation is not yet implemented.
