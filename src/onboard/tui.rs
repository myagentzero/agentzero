...
impl Step {
    ...
    fn previous(self) -> Self {
        let idx = self.index();
        if idx == 0 { self } else { Self::ORDER[idx - 1] }
    }
}
...
pub async fn run_wizard_tui(force: bool) -> Result<Config> {
    ...
    let mut config = run_quick_setup(
        api_key,
        Some(&provider),
        Some(&model),
        Some(&memory_backend),
...
fn draw_ui(frame: &mut Frame<'_>, state: &TuiState) {
    ...
    let footer_text = vec![
        ...
        Line::from(
            "Nav: ↑/↓ or j/k move | Tab/Shift+Tab cycle fields | n/PgDn next step | p/PgUp back",
        ),
        Line::from("Select: ←/→ or h/l or Space toggles/options | Enter runs checks on probe rows"),
        Line::from(
            "Edit: Enter or e opens text editor | Enter or Ctrl+S saves | Esc cancels | Ctrl+U clears",
        ),
...
impl Drop for ScopedEnvVar {
    fn drop(&mut self) {
        // SAFETY: called during single-threaded onboarding teardown.
        unsafe {
