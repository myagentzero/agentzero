...
impl CostObserver {
    ...
    /// Normalize custom provider names by stripping URL suffixes.
    ///
    /// Custom providers arrive as `custom:https://host:port` or
    /// `anthropic-custom:https://host:port`. We collapse them to just
    /// `custom` / `anthropic-custom` so pricing lookups and cost
    /// records use a clean, stable key.
    fn normalize_provider(provider: &str) -> &str {
        if provider.starts_with("custom:") {
            "custom"
        } else if provider.starts_with("anthropic-custom:") {
            "anthropic-custom"
        } else {
            provider
        }
...
impl Observer for CostObserver {
    fn record_event(&self, event: &ObserverEvent) {
        ...
        } = event
        {
            ...
            let provider_label = Self::normalize_provider(provider);
...
mod tests {
    ...
    #[test]
    fn cost_observer_normalizes_custom_provider_url() {
        let (_tmp, tracker) = create_test_tracker();
        let mut prices = HashMap::new();
        prices.insert(
            "custom/my-model".into(),
            ModelPricing {
                input: 1.0,
                output: 2.0,
            },
        ...
        // Provider arrives with full URL from custom provider config
        observer.record_event(&ObserverEvent::LlmResponse {
            provider: "custom:https://providerurl.com:8888".into(),
            model: "my-model".into(),
            duration: Duration::from_millis(100),
            success: true,
            error_message: None,
    ...
    #[test]
    fn cost_observer_normalizes_anthropic_custom_provider_url() {
        let (_tmp, tracker) = create_test_tracker();
        let mut prices = HashMap::new();
        prices.insert(
            "anthropic-custom/claude-sonnet-4-20250514".into(),
            ModelPricing {
                input: 3.0,
                output: 15.0,
            },
        ...
        observer.record_event(&ObserverEvent::LlmResponse {
            provider: "anthropic-custom:https://internal-proxy.corp:9090".into(),
            model: "claude-sonnet-4-20250514".into(),
            duration: Duration::from_millis(100),
            success: true,
            error_message: None,
    ...
    #[test]
    fn normalize_provider_strips_custom_urls() {
        assert_eq!(
            CostObserver::normalize_provider("custom:https://example.com:8888"),
            "custom"
        );
        assert_eq!(
            CostObserver::normalize_provider("anthropic-custom:https://proxy.internal"),
            "anthropic-custom"
        );
        assert_eq!(CostObserver::normalize_provider("openai"), "openai");
        assert_eq!(CostObserver::normalize_provider("anthropic"), "anthropic");
    }
