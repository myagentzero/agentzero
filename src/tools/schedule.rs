...
impl Tool for ScheduleTool {
    ...
    fn description(&self) -> &str {
        "Manage scheduled shell-only tasks. Actions: create/add/once/list/get/cancel/remove/pause/resume. \
         ...
         To send a scheduled message to Discord/Telegram/Slack/Email, use the cron_add tool with job_type='agent' \
...
mod tests {
    ...
    async fn medium_risk_create_requires_approval() {
        ...
        // Without approved=true, the tool-level validation should reject
        let denied = tool
            .execute(json!({
                "action": "create",
                "expression": "*/5 * * * *",
