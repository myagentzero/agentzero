...
async fn normalize_remote_image(
    ...
    let service_keys = build_remote_fetch_service_keys(provider_hint);
...
async fn normalize_remote_image_once(
    ...
    let request = remote_client.get(source).header(
        reqwest::header::USER_AGENT,
        crate::config::schema::DEFAULT_USER_AGENT,
    );
...
fn build_remote_fetch_service_keys(provider_hint: Option<&str>) -> Vec<String> {
...
mod tests {
    ...
    fn build_remote_fetch_service_keys_deduplicates_service_candidates() {
        let keys = build_remote_fetch_service_keys(Some("ollama"));
        assert_eq!(
            keys,
            vec![
