...
impl Channel for SlackChannel {
    ...
    async fn start_typing(&self, recipient: &str) -> anyhow::Result<()> {
        let thread_ts = {
            ...
            match map.get(recipient) {
                Some(ts) => ts.clone(),
                // No assistant thread context for this channel — nothing to do.
                None => return Ok(()),
            }
        };
    ...
    async fn stop_typing(&self, _recipient: &str) -> anyhow::Result<()> {
        // Slack auto-clears assistant status when the bot sends a message via
        // chat.postMessage, so no explicit clear is needed.
        Ok(())
    }
